...
describe("input:number", () => {
  ...
  test("min is props.min", () => {
    render(<ColorRangeInput setVal={vi.fn()} min={42} />);

    const target = screen.getByRole("spinbutton");
    expect(target.getAttribute("min")).toBe("42");
  });
  test("max is props.max", () => {
    render(<ColorRangeInput setVal={vi.fn()} max={123} />);
  ...
  describe("value", () => {
    test("INIT, then props.value", () => {
      render(<ColorRangeInput setVal={vi.fn()} value={85} />);
    ...
    test("props.name exists, then input.name is props.name", () => {
      render(
        <ColorRangeInput setVal={vi.fn()} value={85} name="testFormName" />
      );
      const target = screen.getByRole("spinbutton");
...
describe("input:range", () => {
  ...
  test("min is props.min", () => {
    render(<ColorRangeInput setVal={vi.fn()} min={23} />);
  ...
  test("max is props.max", () => {
    render(<ColorRangeInput setVal={vi.fn()} max={94} />);
  ...
  describe("isHue", () => {
    test("isHue true, then --slide-color is localValue", () => {
      render(<ColorRangeInput setVal={vi.fn()} value={44} isHue={true} />);
    ...
    test("isHue fals, then --slide-color is empty", () => {
      render(<ColorRangeInput setVal={vi.fn()} value={44} isHue={false} />);
      const input = screen.getByRole("spinbutton");
      fireEvent.focus(input);
